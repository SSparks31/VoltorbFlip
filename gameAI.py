...
class Game(game.Game):
    def heuristics_first(self, i, j): # Regra 1: Linhas sem possibilidade de voltorb
        ...
        if voltorbsFoundLine + flippedLine == 4:
            possibilitiesLine.intersection_update((pointsLine - pointsFoundLine,))
        if voltorbsLine == voltorbsFoundLine:
            possibilitiesLine.intersection_update(range(1, pointsLine  + flippedLine - pointsFoundLine - 3))
        if voltorbsFoundColumn + flippedColumn == 4:
            possibilitiesColumn.intersection_update((pointsColumn - pointsFoundColumn,))
        if voltorbsColumn == voltorbsFoundColumn:
            possibilitiesColumn.intersection_update(range(1, pointsColumn + flippedColumn - pointsFoundColumn - 3))
    ...
    def heuristics_third(self, i, j): # Cálculo de todas as possibilidades
        ...
        pointsMissingLine = pointsLine - pointsFoundLine
        pointsMissingColumn = pointsColumn - pointsFoundColumn
        ...
        # Regra 1: Apenas valores
        if voltorbsLine == voltorbsFoundLine:
            possibilitiesLine.intersection_update((1, 2, 3))
            if unflippedLine == 1:
                possibilitiesLine.intersection_update((pointsMissingLine,))
        if voltorbsColumn == voltorbsFoundColumn:
            possibilitiesColumn.intersection_update((1, 2, 3))
            if unflippedColumn == 1:
                possibilitiesColumn.intersection_update((pointsMissingColumn,))

        # Regra 2: Apenas Voltorbs
        if unflippedLine == voltorbsLine - voltorbsFoundLine:
            possibilitiesLine.intersection_update(('X',))
        if unflippedColumn == voltorbsColumn - voltorbsFoundColumn:
            possibilitiesColumn.intersection_update(('X',))

        # Regra 3: "Soma 5"
        if unflippedLine + pointsFoundLine + voltorbsFoundLine == pointsLine + voltorbsLine:
            possibilitiesLine.intersection_update(('X', 1))
        if unflippedColumn + pointsFoundColumn + voltorbsFoundColumn == pointsColumn + voltorbsColumn:
            possibilitiesColumn.intersection_update(('X', 1))

        # Regra 4: Há apenas um valor na linha
        if voltorbsLine == 4:
            possibilitiesLine.intersection_update(('X', pointsLine))
        if voltorbsColumn == 4:
            possibilitiesColumn.intersection_update(('X', pointsColumn))

        # Regra 5: "Soma 6"
        if unflippedLine + pointsFoundLine + voltorbsFoundLine == pointsLine + voltorbsLine + 1:
            possibilitiesLine.intersection_update(('X', 1, 2))
        if unflippedColumn + pointsFoundColumn + voltorbsFoundColumn == pointsColumn + voltorbsColumn + 1:
            possibilitiesColumn.intersection_update(('X', 1, 2))
        
        # Regra 6: Sem possibilidade de 1
        if (pointsLine - pointsFoundLine - 2)//3 >= 5 - voltorbsLine:
            possibilitiesLine.intersection_update(('X', 2, 3))
        if (pointsColumn - pointsFoundColumn - 2)//3 >= 5 - voltorbsColumn:
            possibilitiesColumn.intersection_update(('X', 1, 2))
    ...
    def play(self): ## Sobrescreve play() do jogo original para operar utilizando IA
        ...
        while any([_ == ' ' for line in self._board for _ in line]):
            ...
            if all([set(('X' , 1)) == _[1] for _ in heuristics]):
                ...
                return
            safeMovements = [_ for _ in heuristics if 'X' not in _[1] or len(_[1]) == 1]
            if safeMovements:
